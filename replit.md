...
-   **Key Modules & Features**:
    ...
    -   **Defects Export (Excel + PDF)**: `POST /api/defects/export` endpoint in `export_router.py`. Supports scope `unit` or `building` with `format` parameter (`excel` or `pdf`, validated). Server-side filtering matching frontend semantics (status groups differ by scope: building "open" = all non-closed; apartment "open" = open/assigned/reopened, "in_progress" = separate group). **Excel**: `.xlsx` with Hebrew headers, RTL sheet, amber header row, auto-filter, image URLs as clickable hyperlinks. **PDF**: A4 pages with Rubik font, Hebrew RTL via arabic_reshaper+bidi, embedded images (max 4 per defect, resized to max 7x5cm, SSRF-protected with host allowlist), filter summary, defect cards with all fields. Frontend: `exportService.exportDefects()` in `api.js` (blob download with format param), `ExportModal` component (Radix Dialog) with format picker (Excel/PDF toggle, amber highlight), export button ("ייצוא") on both defect pages. Dependencies: `openpyxl`, `reportlab`, `arabic_reshaper`, `python-bidi`, `Pillow`.
