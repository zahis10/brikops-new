...
-   **Key Modules & Features**:
    ...
    -   **QC (Quality Control) System**: Floor-level and unit-level quality control with a hardcoded template (v4), item management, RBAC, stage-based workflow, and an approver system. Supports scope model (`scope: 'floor'|'unit'`) on template stages and qc_runs. Unit-scoped stage: `stage_tiling` (9 items, scope='unit') — tiling is inspected per apartment. Endpoints: `GET /api/qc/units/{unit_id}/run` (get/create unit run with backfill), `GET /api/qc/floors/{floor_id}/units-status` (per-unit tiling progress). Stages include `is_completable` computed field. MongoDB indexes on `qc_runs` for `floor_id+template_id` and `unit_id+template_id`. Batch-status only includes floor-scope stages. UnitQCSelectionPage lists units with progress; StageDetailPage supports unit mode via navigation state (returnTo, unitName, scope).
