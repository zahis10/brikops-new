...
-   **Router Architecture**: The main `router.py` has been refactored into sub-routers (all use `prefix="/api"`):
    ...
    -   `debug_router.py`: 20 debug/health/admin endpoints (all gated behind `require_super_admin` except `/api/health`). Includes `POST /debug/whatsapp-send-test` (SA-only, 3 modes: `template_text_only`/`template`/`template_with_meta_media_id`), `GET /debug/whatsapp-latest?limit=N` (SA-only, returns config + recent notification_jobs + whatsapp_events), `POST /debug/whatsapp-test` (legacy SA-only), and `GET /debug/notification-lookup?phone=<fragment>` (debug-access-gated).
...
-   **Key Modules & Features**:
    -   **Authentication & Onboarding**: Supports email/password, phone OTP, PM approval, WhatsApp Magic Link Login, and comprehensive onboarding. Includes a dev-only login endpoint for demo users. **Deep-link preservation**: `ProtectedRoute` saves full intended URL (pathname+search+hash) to `sessionStorage` key `intendedPath` + marks `deepLinkSource=external`; all login flows (email, OTP, demo, WA login) restore the target after auth. WA login (`WaLoginPage.js`) reads `intendedPath` after JWT hash exchange instead of hard-redirecting to `/`. `TaskDetailPage` detects external entry via `deepLinkSource` and sets contractor back-target to `/projects/{projectId}/tasks?assignee=me`. 403/404 errors show inline error states (not `navigate('/projects')`) with priority-ordered back button: `taskDetailReturnTo` > contractor filtered URL > `/projects`.
    -   **Multi-channel Communication**: Automated notifications via WhatsApp (with SMS fallback) using Meta-approved templates, configurable by user's preferred language. Fallback image (`WA_FALLBACK_IMAGE_URL` env var, default `s3://public/assets/brikops-logo.png`) generates a fresh presigned S3 URL at send time when a defect has no attached photo, ensuring the required IMAGE header always returns a valid `image/png` Content-Type. S3 client uses `s3v4` signature with `virtual` addressing style to ensure presigned URLs include the correct region subdomain. `_make_absolute_url()` handles `s3://` references by calling `generate_url()` to produce presigned HTTPS URLs (fixes "image.link is not a valid link" Meta error). Image header validation: task attachment URL must be `https://`; otherwise falls back to S3 logo; `[WA:IMAGE]` log shows source (task_attachment/fallback/none). SMS fallback task links always use `get_public_base_url()` for full `https://app.brikops.com/tasks/{id}` URLs. **Template names are ENV-configurable** (`WA_DEFECT_TEMPLATE_HE/EN/AR/ZH`) with v1 defaults; switching to v2 templates requires only an env var change, no deploy. Body params order: `{{1}}=ref, {{2}}=location, {{3}}=issue`. `WA_TEMPLATE_PARAM_MODE` env var (`named`/`positional`) controls whether `parameter_name` fields are included (named=v1 compat, positional=v2). Login template configurable via `WA_LOGIN_TEMPLATE_HE` (default: `brikops_login_link_he`). All links use `PUBLIC_APP_URL=https://app.brikops.com`.
    ...
    -   **Proof Management**: Multi-image proof uploads with audit logging. Client-side image compression (>800KB → max 1600px width, JPEG 0.7 quality) reduces upload payload from ~5-10MB to ~200-400KB. Upload axios timeout set to 55s with specific error messages for 413 and timeout errors. All FormData uploads in `api.js` omit explicit `Content-Type` header (browser auto-sets `multipart/form-data` with correct boundary). Frontend logs image sizes (original/compressed) and per-image upload success/failure. Backend `upload_task_attachment` logs content-type boundary presence and timing at each stage (`[ATTACH:ENTER]`, `[ATTACH:STORED]`, `[ATTACH:COMPLETE]`). **Defect creation flow** (`NewDefectModal.js` `handleSubmit`): Step 1 create task → Step 2 assign contractor + WhatsApp (critical, blocks on failure) → Step 3 upload images (non-blocking, uses `Promise.allSettled`, warns on failure but task is already assigned). Modal closes and form resets after Step 2 succeeds. Image upload failures never prevent task creation or contractor assignment. **WhatsApp toast truth**: `sent`/`queued` → `toast.info` (amber, "בהמתנה למסירה"); only `delivered` (from webhook) → `toast.success` (green); `failed` → `toast.error` with reason. **CameraModal** (`frontend/src/components/CameraModal.js`): getUserMedia-based inline camera with `facingMode: {ideal: 'environment'}`, `<video autoPlay playsInline muted>` for iOS, canvas capture to JPEG blob, stream track cleanup on close/unmount. Error reporting: shows specific Hebrew message per `err.name` (`NotAllowedError`/`NotReadableError`/`NotFoundError`/`OverconstrainedError`), debug line `camera_error=<err.name>`, and iOS help steps (Safari aA → Website Settings → Camera → Allow; iOS Settings → Safari → Camera; Reset via Website Data). `OverconstrainedError` auto-retries with `{ video: true }` fallback. Integrated into both NewDefectModal and TaskDetailPage contractor proof upload.
    -   **WhatsApp Notification Debug**: Assign endpoint returns `notification_status` with `to_phone_masked` (last 4 digits: `+972****6456`), `provider_message_id`, `provider_status` (sent/failed/skipped_dry_run/duplicate), and `error` reason. Frontend toasts reflect real delivery state: "בהמתנה למסירה" for sent, "נמסר" for delivered, "נכשל" with reason for failed. TaskDetailPage notification history shows message_id, delivery state, channel badge (WhatsApp/SMS), updated_at, and "העתק דיבוג" button for managers. Phone normalization: Israeli 050... numbers auto-normalized to E.164 (+9725...) at resolve time. Missing/invalid phone: explicit failure with "אין מספר וואטסאפ תקין לקבלן".
