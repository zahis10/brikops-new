...
-   **Plans Archive & Replace System**: Phases 1-3 implemented. Phase 1: archive-first (no delete). Phase 2: replace flow with automatic archival. Phase 3: version history/timeline view. Backend endpoints: `GET /plans` (active only), `GET /plans/archive`, `GET /plans/{id}/history`, `PATCH /plans/{id}/archive`, `PATCH /plans/{id}/restore`, `POST /plans/{id}/replace`. History endpoint walks the `replaces_plan_id` chain backward with cycle guard (max 50). Permissions: same `PLAN_UPLOAD_ROLES` (project_manager, management_team) for archive/restore/replace; all roles can view history; contractor/viewer read-only on all pages. Unit plans archive/history not yet implemented (intentional deferral).
