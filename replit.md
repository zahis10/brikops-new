...
-   **Key Modules & Features**:
    -   **Authentication & Onboarding**: Supports email/password, phone OTP, PM approval, WhatsApp Magic Link Login, and comprehensive onboarding, including deep-link preservation across login flows. Onboarding route (`/onboarding`) uses feature flag `onboarding_v2` (fetched from public `/api/config/features`) with layered access control: registration link on LoginPage is always visible and navigates to `/onboarding?mode=register` (durable query param survives refresh). OnboardingPage allows access when ANY valid context is present: `?mode=register` (registration link click), `incomingPhone` (OTP flow router state), or invite token. Only bare `/onboarding` with zero context redirects to `/login`. Invite flows with onboarding disabled show blocked message with login button.
    ...
    -   **Defects V2 (Phase 1 — Read-Only)**: Parallel building-level and apartment-level defect views, gated by `ENABLE_DEFECTS_V2` feature flag (default `true` in dev, `false` in prod). Exposed via `/api/config/features` as `defects_v2`. Backend endpoint `GET /api/buildings/{building_id}/defects-summary` aggregates task defect counts per unit per floor (tasks collection only, no QC). Frontend pages: `BuildingDefectsPage` (floor rows with apartment tiles, color-coded badges red≥3/amber1-2/green/gray) at `/projects/:projectId/buildings/:buildingId/defects`; `ApartmentDashboardPage` (KPI summary, filter chips, defect list) at `/projects/:projectId/units/:unitId/defects`. Entry point: "ליקויים" button on building rows in ProjectControlPage structure tab (hidden when flag OFF). Both pages redirect to old flow when flag OFF. Old defects flow (`/projects/:id/tasks`, `/units/:id/tasks`) fully intact.
