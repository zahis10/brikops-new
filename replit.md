...
-   **ProjectPlansPage**: Refreshed project-level plans page at `/projects/:projectId/plans`. Mobile-first layout with compact dark header (project name subtitle), client-side search, horizontal scrollable discipline filter chips (amber active state), clean plan rows with filename/discipline/date/note, upload modal, inline add-discipline, and smart back navigation. No sidebar.
