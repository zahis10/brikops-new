...
-   **Nginx Proxy Config**: `.platform/nginx/conf.d/proxy.conf` configures the AWS EB nginx reverse proxy: `client_max_body_size 20M`, `proxy_read_timeout 120s`, `proxy_send_timeout 120s`, `proxy_connect_timeout 120s`. Without this, EB uses nginx defaults which may reject/timeout multipart file uploads before they reach the FastAPI app (returning errors without CORS headers, causing browser "Network Error").
