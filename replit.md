...
-   **Key Modules & Features**:
    ...
    -   **Excel Export (Defects V2)**: `POST /api/defects/export` endpoint in `export_router.py`. Supports scope `unit` or `building` with server-side filtering matching frontend semantics (status groups differ by scope: building "open" = all non-closed; apartment "open" = open/assigned/reopened, "in_progress" = separate group). Generates `.xlsx` with Hebrew headers, RTL sheet, amber header row, auto-filter, image URLs as clickable hyperlinks (multi-image: separate columns). Frontend: `exportService.exportDefects()` in `api.js` (blob download), `ExportModal` component (Radix Dialog), export button ("ייצוא") on both defect pages next to filter button. Dependency: `openpyxl`.
