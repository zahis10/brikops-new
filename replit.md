...
-   **Key Modules & Features**:
    ...
    -   **Admin Analytics**: `GET /api/admin/analytics/user-activity?period=7|30|90&role=&org_id=&sort=&order=&page=1&limit=50&search=` returns paginated per-user platform-wide activity (defects, QC, handover, photos, WA), activity score (0-100), status (active/low/dormant/never). Score weights: login_recency 30, defects_created 15, defects_closed 15, qc_checked 15, handover 10, photos 10, whatsapp 5. `GET /api/admin/analytics/feature-usage?period=7|30|90` returns per-feature adoption stats (defects, QC, handover, WhatsApp, plans — annotation deferred). Frontend: AdminActivityPage with sortable/filterable/paginated user table and feature usage card grid. Accessible via "פעילות" tab in AdminPage. Route: `/admin/activity`.
