...
-   **Key Modules & Features**:
    ...
    -   **Handover Protocol System (Phase 1a+1b+1b-forms)**: Unit-level handover protocol management for initial and final inspections. `handover_router.py` with 11 endpoints. Frontend Phase 1b (#73+#74): HandoverTabPage (states A-E from backend data), HandoverProtocolPage (progress ring, expandable engine-managed form sections, template section cards with per-section progress rings), HandoverSectionPage (item list with status icons, mark-all-ok with confirmation, item modal), HandoverItemModal (Drawer bottom sheet with 5 status buttons, notes field, defect creation with 409 handling, defect badge+link). Engine-managed forms in `frontend/src/components/handover/`: HandoverPropertyForm (8 fields), HandoverTenantForm (multi-tenant with add/remove), HandoverMeterForm (water+electricity readings, final type only), HandoverDeliveredItems (quantity+notes per item, final type only), HandoverLegalText (legal_text_edited flag from backend only), HandoverGeneralNotes (apartment/storage/parking). All forms save via PUT protocol update, all read-only when signed. Routes: `/projects/:projectId/units/:unitId/handover`, `/handover/:protocolId`, `/handover/:protocolId/sections/:sectionId`. "מסירה" card on UnitHomePage (purple). `handoverService` in api.js with all endpoints. Template resolution: project `handover_template_version_id` → DB default (type=handover, is_default=true) → HANDOVER_TEMPLATE constant (15 sections, ~145 items). Protocol status flow: draft→in_progress→signed. Auto-transitions draft→in_progress on first item update. Signed protocols are locked (403 on all mutations). Only super_admin can reopen with reason. `handover_protocols` collection with unique index on (project_id, unit_id, type). Initial protocols have no meters/delivered_items; final protocols include both. Legal text sourced from org fields (`default_handover_legal_text_initial`/`_final`). `create-defect` creates a task with `source: "handover_initial"|"handover_final"` and links back via `defect_id` on the item (409 if already exists). Summary endpoint provides per-building breakdown with status counts + open handover defects. Signatures section placeholder remains for Task #75.
