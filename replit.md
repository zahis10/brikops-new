...
-   **Key Modules & Features**:
    ...
    -   **QC Template Management (Phase 1)**: `qc_templates` collection with family-based versioning — each edit creates a new immutable version document. Default template seeded from FLOOR_TEMPLATE constant on startup (idempotent). Admin CRUD endpoints: list families, get version, create family, edit (new version), clone, assign to project. New runs store `template_version_id` metadata. `_get_template()` still returns hardcoded FLOOR_TEMPLATE (Phase 2 will wire dynamic resolution). Admin UI at `/admin/qc-templates` with template list + inline stage/item editor. Project assignment stores specific version ID (not family).
