...
-   **Key Modules & Features**:
    ...
    -   **QC (Quality Control) System**: Floor-level quality control with a hardcoded template, item management, RBAC, stage-based workflow, and an approver system. Supports scope model (`scope: 'floor'|'unit'`) on template stages and qc_runs for future unit-scoped stages (e.g., tiling). New endpoint `GET /api/qc/units/{unit_id}/run` for unit-scoped runs. Stages include `is_completable` computed field. MongoDB indexes on `qc_runs` for `floor_id+template_id` and `unit_id+template_id`.
