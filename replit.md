...
-   **Key Modules & Features**:
    ...
    -   **Billing System**: Simplified license + per-unit pricing (₪450 per project + ₪15/unit/month; founder_6m=₪499 flat; manual_override passthrough). PayPlus payment gateway for credit card payments (sandbox-first), Green Invoice for invoice generation, and recurring renewal infrastructure. Plan selection: Founder (₪499/mo × 6, 1 project, 30 slots max) vs Standard. `set_org_plan()` shared function for plan activation. Webhook amount verification (non-blocking). Founder expiry via daily cron + checkout auto-switch. Admin founder toggle via `app_config` collection.
    ...
    -   **Unit Quota Enforcement**: `project.total_units` acts as an absolute cap on active units. All 7 creation/restore paths (create_unit, create_floor with auto-units, bulk_create_units, excel import, restore_unit/floor/building) enforce the cap. Exceeding triggers auto-creation of `unit_quota_request` for admin approval. Admin can approve (updates `total_units`) or reject. Frontend shows X/N counter and disables add buttons at cap. Legacy projects (no `total_units`) are unaffected. No super_admin bypass — must update `total_units` via admin endpoint first.
    ...
    -   **Full Project Excel Export (Multi-Sheet)**: Single Excel file with 6 sheets (ליקויים, פרוטוקולי מסירה, בקרת ביצוע, מבנה פרויקט, צוות, חברות וקבלנים). Immediate StreamingResponse download, no background job. PM/super_admin only. Audit event: `full_excel_export`. Endpoint: `POST /api/projects/{id}/export/excel`. Blue card in `ProjectDataExportTab.js`.
