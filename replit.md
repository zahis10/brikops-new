...
-   **Key Modules & Features**:
    ...
    -   **G4 Tenant Data Import (Task #96)**: Excel/CSV import of tenant (buyer) data for handover protocols. Backend: `import_router.py` with 3 endpoints — `GET .../template` (static xlsx download from `backend/static/g4_template.xlsx`), `POST .../preview` (parse xlsx/csv with fixed columns A-I, validate, check overwrites), `POST .../execute` (re-validate server-side, upsert to `unit_tenant_data` collection). Data stored in `unit_tenant_data` MongoDB collection with compound unique index on (project_id, building_name, floor, apartment_number). Phone normalization via existing `normalize_israeli_phone`. Tenant prefill in `create_protocol` queries `unit_tenant_data` before insert — populates tenants array with imported data (including tenant_2). Frontend: `G4ImportModal` component with 3-step flow (upload → preview table with validation highlighting → execute with overwrite warnings). "ייבוא רוכשים" button on HandoverOverviewPage. Auth: PM, owner, super_admin, management_team. Existing protocols never modified by import.
    -   **Admin CS Dashboard (Task #98)**: Read-only analytics dashboard at `/admin/dashboard` for super_admin. Backend: `admin_dashboard.py` with `GET /admin/dashboard/stats` endpoint — uses separate lightweight MongoDB aggregations (buildings, units via floors, tasks, protocols, invoices, subscriptions, project_billing) combined in Python. No N+1 queries. Returns: summary KPIs (active orgs, projects, units, monthly revenue), alerts (dormant orgs, overdue invoices, trial ending, no projects), activity chart data, churn distribution, full org table data. Frontend: `AdminDashboardPage.js` with 4 KPI cards, collapsible alerts section, recharts BarChart (activity per org) + PieChart (churn donut), sortable/filterable org table with 13 columns. Filter chips for churn status. Hebrew RTL throughout. `last_login_at` field added to users on password login (`auth_router.py`) and WhatsApp magic link login (`wa_login.py`). Churn logic: active (<7d), at_risk (7-30d), dormant (>30d or inactive sub), unknown (no login data → "לא ידוע"). Monthly cost = sum of active `project_billing.monthly_total` per org (matches OrgBillingPage). Navigation tab "דשבורד CS" added to AdminPage.
