...
Before applying an append, check that the complete context anchor occurs exactly once and ends at the original EOF. An approval marker alone can silently select an earlier section even when the intended patch is described as an append. Include any separator bytes in the standalone section and byte-delta calculation, especially when the original file has no trailing newline.
