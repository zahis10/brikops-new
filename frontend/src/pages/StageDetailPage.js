        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-sm border-t border-slate-200 shadow-lg"
          ...
          <div className="max-w-2xl mx-auto px-4 pt-3 space-y-2">
            {validationErrors.length > 0 && (
              <div className="bg-red-50 border border-red-300 rounded-xl p-2.5">
                <div className="flex items-center gap-2 text-red-700 font-bold text-xs mb-1">
                  <AlertCircle className="w-3.5 h-3.5" />
                  לא ניתן לשלוח — יש שדות חסרים
                </div>
                <ul className="space-y-0.5 pr-2" dir="rtl">
                  ...
                  {validationErrors.length > 4 && (
                    <li className="text-[10px] text-red-500">...ועוד {validationErrors.length - 4}</li>
                  )}
                </ul>
              </div>
            )}
            ...
            {canSubmitOrEdit && !submitBlockers.canSubmit && !hasChanges && validationErrors.length === 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-2.5">
                <div className="flex items-center gap-2 text-amber-800 font-bold text-xs mb-1">
                  <AlertCircle className="w-3.5 h-3.5" />
                  לא ניתן לשלוח לאישור
                </div>
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-amber-700 mb-1.5" dir="rtl">
                  {submitBlockers.rejectedItems > 0 && (
                    <span className="text-red-700 font-bold"><ShieldX className="w-3 h-3 inline ml-0.5" />{submitBlockers.rejectedItems} סעיפים נדחו בביקורת</span>
                  )}
                  {submitBlockers.unmarked > 0 && (
                    <span><span className="font-bold">{submitBlockers.unmarked}</span> סעיפים לא סומנו</span>
                  )}
                  {submitBlockers.missingPhotos > 0 && (
                    <span>חסרות תמונות ב-<span className="font-bold">{submitBlockers.missingPhotos}</span> סעיפים</span>
                  )}
                  {submitBlockers.missingNotes > 0 && (
                    <span>חסרות הערות ב-<span className="font-bold">{submitBlockers.missingNotes}</span> סעיפים</span>
                  )}
                </div>
                <div className="flex gap-2">
                  {submitBlockers.firstMissingId && (
                    <button onClick={scrollToFirstMissing}
                      className="flex items-center gap-1.5 text-xs font-medium text-amber-800 bg-amber-100 hover:bg-amber-200 rounded-lg px-3 py-2 min-h-[40px] transition-all">
                      <Navigation className="w-3.5 h-3.5" />
                      לסעיף הראשון
                    </button>
                  )}
                  {activeFilter === 'all' && (submitBlockers.rejectedItems > 0 || submitBlockers.unmarked > 0 || submitBlockers.missingPhotos > 0 || submitBlockers.missingNotes > 0) && (
                    <button onClick={() => setActiveFilter('problems')}
                      className="flex items-center gap-1.5 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 rounded-lg px-3 py-2 min-h-[40px] transition-all">
