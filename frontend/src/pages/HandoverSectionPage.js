...
const HandoverSectionPage = () => {
  ...
  const advanceAfterSave = useCallback((currentItemId) => {
    ...
    if (nextId) {
      ...
      setTimeout(() => {
        forceCloseSheet();
        toast.success('הסקשן הושלם! ✓', { duration: 2000 });
      }, 200);
    }
  ...
  }, [items, isSigned, projectId, protocolId, sectionId, updateItemLocally, loadProtocol]);

  const handleResetSection = useCallback(async () => {
    setShowBatchConfirm(null);
    const checked = items.filter(i => i.status && i.status !== 'not_checked');
    if (checked.length === 0 || isSigned) return;
    try {
      setMarkingAll(true);
      await handoverService.batchUpdateItems(projectId, protocolId, sectionId, {
        item_ids: checked.map(i => i.item_id),
        status: 'not_checked',
      });
      checked.forEach(i => updateItemLocally(i.item_id, { status: 'not_checked' }));
      toast.success('הסקשן אופס בהצלחה');
    } catch (err) {
      console.error(err);
      toast.error(t('handover', 'updateError'));
      await loadProtocol();
    ...
    <div className="min-h-screen bg-slate-50" dir="rtl">
          ...
          <div className="flex gap-2">
            {nonOkCount > 0 && (
              <button
                ...
                disabled={markingAll}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm font-medium hover:bg-green-100 active:scale-[0.98] transition-all disabled:opacity-50"
              >
            ...
            {checkedCount > 0 && (
              <button
                ...
                disabled={markingAll}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-100 active:scale-[0.98] transition-all disabled:opacity-50"
              >
      ...
      {showBatchConfirm && (
        <div className="max-w-lg mx-auto px-4 mt-2">
          <div className={`rounded-xl p-3 flex items-center justify-between border ${
            showBatchConfirm === 'markAll' ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'
          }`}>
            ...
            <div className="flex gap-2">
              <button
                onClick={showBatchConfirm === 'markAll' ? handleMarkAllOk : handleResetSection}
                disabled={markingAll}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium text-white disabled:opacity-50 ${
                  showBatchConfirm === 'markAll' ? 'bg-green-500 hover:bg-green-600' : 'bg-slate-500 hover:bg-slate-600'
                }`}
              >
                אישור
              </button>
          ...
          >
            ...
            >
              ...
              >
                ...
                {!isDefectStatus && !isSigned && (status === 'ok' || status === 'not_relevant') && (
      ...
      {showUnsavedConfirm && (
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-5 max-w-sm w-full shadow-xl" dir="rtl">
            <p className="text-sm font-medium text-slate-800 mb-4">יש שינויים שלא נשמרו. לבטל?</p>
            <div className="flex gap-2">
              <button
