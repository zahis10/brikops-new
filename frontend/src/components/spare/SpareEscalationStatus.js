...
export default function SpareEscalationStatus({ unitData, onOpenSheet }) {
  ...
  return <div className="mb-3 flex flex-wrap items-center gap-2">{esc && <p className={esc.status === 'done' ? 'text-xs font-medium text-emerald-700' : 'text-xs text-slate-500'}>{esc.status === 'done' ? '✓ טופל' : 'סומן כלא רלוונטי'} ע״י {esc.resolved_by?.name} · {esc.resolved_at ? new Date(esc.resolved_at).toLocaleDateString('he-IL') : ''}{esc.resolution_note ? ` · "${esc.resolution_note}"` : ''}</p>}{canSend && <button onClick={() => onOpenSheet('new')} className="inline-flex min-h-[44px] items-center gap-1.5 rounded-lg border border-amber-300 px-2.5 py-1.5 text-xs font-bold text-amber-700 hover:bg-amber-50"><Bell className="h-3.5 w-3.5" />הקפץ למנהל הפרויקט</button>}</div>;
}